...
func mineLoop(syncerRPCClient, syncedRPCClient *rpc.Client) error {
	...
	for i := uint64(0); i < activeConfig().NumberOfBlocks; i++ {
		...
		if err != nil {
			...
			// TODO: Fix the race condition in danaminer so it won't panic (proper shutdown handler)
...
func mineBlock(syncerRPCAddress string, miningAddress util.Address) error {
	danaMinerCmd, err := common.StartCmd("MINER",
		"danaminer",
	...
	return errors.Wrapf(danaMinerCmd.Wait(), "error with command '%s'", danaMinerCmd)
}
