...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("danadsanity-danad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("danad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "DANAD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "DANAD-STDERR")
