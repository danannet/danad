...
import (
	...
	"github.com/danannet/danad/domain/consensus/utils/mining"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("danad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("danad datadir: %s", dataDir)

	danadCmd, err := common.StartCmd("DANAD",
		"danad",
	...
	spawn("startNode-danadCmd.Wait", func() {
		err := danadCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("danadCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes danad closes ungracefully
				log.Errorf("danadCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(danadCmd, "danadCmd")
