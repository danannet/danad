...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		danadMessage, err := client.Post(&protowire.DanadMessage{Payload: &protowire.DanadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := danadMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	danadMessage := &protowire.DanadMessage{}
	err := protojson.Unmarshal([]byte(response), danadMessage)
	if err != nil {
	...
	return marshalOptions.Format(danadMessage)
}
