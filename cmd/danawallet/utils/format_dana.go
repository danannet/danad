...
func KasToSompi(amount string) (uint64, error) {
	...
	if constants.SompiPerDana%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerDana is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerDana))
	decimalStr := ""
