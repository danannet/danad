...
import (
	...
	"github.com/danannet/danad/infrastructure/os/execenv"
	"github.com/danannet/danad/infrastructure/os/limits"
	"github.com/danannet/danad/infrastructure/os/signal"
	"github.com/danannet/danad/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "danadsvc",
	DisplayName: "Danad Service",
	Description: "Downloads and stays synchronized with the Dana blockDAG and " +
		"provides DAG services to applications.",
}

type danadApp struct {
	cfg *config.Config
}

// StartApp starts the danad app, and blocks until it finishes running
func StartApp() error {
...
func (app *danadApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start danad: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down danad...")
		...
		log.Infof("Danad shutdown complete")
	}()
