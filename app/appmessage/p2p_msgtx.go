...
// Outpoint defines a dana data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new dana transaction outpoint point with the
...
// TxIn defines a dana transaction input.
type TxIn struct {
...
// NewTxIn returns a new dana transaction input with the provided
...
// TxOut defines a dana transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new dana transaction output with the provided
...
// MsgTx implements the Message interface and represents a dana tx message.
